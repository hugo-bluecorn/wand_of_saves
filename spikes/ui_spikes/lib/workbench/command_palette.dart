...
/// The one control that reaches every field, proficiency and item on the
...
class CommandPalette extends StatelessWidget {
  ...
  /// Held by the screen so it can be opened by Ctrl+K, and pre-filled at boot
  /// for a capture.
  ...
  /// Restricts the corpus to items, for the inventory screen.
  final bool itemsOnly;

  @override
  Widget build(BuildContext context) {
    final hint = itemsOnly
        ? 'Find an item…'
        : 'Find a field, a proficiency or an item…';
    return SearchAnchor(
  ...
  List<Subject> _startHere() {
    if (itemsOnly) {
      return [
        for (final finding in itemFindingsFor(character)) ?finding.subject,
        for (final item in character.equipped) ItemSubject(item),
      ];
    }
  ...
  List<Subject> _matches(String query) {
    final corpus = <Subject>[
      ...
      for (final item in character.equipped) ItemSubject(item),
      for (final item in character.backpack) ItemSubject(item),
    ];
    return [
...
bool _matchesQuery(Subject subject, String query) {
  ...
  final where = switch (subject) {
    ...
    ItemSubject(:final item) => item.slot ?? 'Backpack',
  };
