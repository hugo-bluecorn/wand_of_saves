...
| Haeravon's *BG:EE FAQ/Walkthrough* (GameFAQs, plain text). On this machine: `~/Downloads/Baldur's Gate  Enhanced Edition FAQ_Walkthrough for PC by Haeravon/*.txt` — §4.2 multi-classing, §6.3 Constitution, §10.1 THAC0 by class/level. | A player's guide carrying the AD&D 2e rules tables in prose — abilities, hit points, THAC0 and armour class by class and level. Copyright its author. | **Cross-check only, never a source.** Facts may be taken from it and expression may not (D1): the numbers are TSR's rules, not the author's writing. Where it and a 2DA disagree, the 2DA wins — it is the game's own data. **And where either disagrees with the running engine, the engine wins**: `HPFM.2da` and this guide both describe a pre-averaged multi-class hit die, and the engine measurably does something else. |
