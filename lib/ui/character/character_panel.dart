...
/// The character sheet, shared by both documents this application opens.
///
/// **One editor, two ViewModels.** MVVM pairs a view with a ViewModel 1:1, so
/// rather than give a savegame and an exported character two sheets that drift
/// apart, the *sheet* is a widget and each ViewModel feeds it. It takes a
/// `Character` and an `onEdit` callback and knows nothing about where either
/// came from — which is what makes "this field only works when I open the save"
...
class CharacterPanel extends ConsumerWidget {
  /// Shows [character], reporting every change through [onEdit].
  ...
  /// How many tabs this panel draws.
  ///
  /// Exposed because the `DefaultTabController` belongs **above** the panel:
  /// in the party shell it sits above the portrait rail so that **the tab
  /// survives changing character**, which is the whole point of having a rail.
  /// A controller owned by the panel would be rebuilt on every selection and
  /// snap back to Character each time.
  static int get tabCount => _CharacterTab.values.length;

  /// The character on screen.
  final Character character;

  /// Called with every change the player asks for.
  ///
  /// ⚠️ **A callback rather than a provider read.** The panel used to reach for
  /// `partyProvider` directly, which tied it to savegames; a `.chr` has no
  /// party, so the coupling had to go before one sheet could serve both.
  final void Function(CharacterEditCommand) onEdit;
  ...
  /// The **party's** reputation, or `null` when there is no party.
  ///
  /// ⚠️ **Not the character's own copy** — see `PartyState.reputation`: three
  /// of four party members carry a stale one and the engine ignores them. An
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    void set(CharacterStat stat, int value) => onEdit(
      SetCharacterStat(
      ...
      children: [
        Padding(
          ...
          child: Row(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              _PortraitButton(character: character, onEdit: onEdit),
...
/// The character's portrait, and the way to change it.
///
/// ⚠️ **A fixed-width edit, so it is as safe as a pip.** The two resrefs
/// exactly fill the gap the spec leaves between `effectVersion` and
/// `reputation`; nothing resizes and the layout pass is not involved. That is
/// what makes this offerable at all on a savegame, where anything that grew
/// would move thirty-nine pointers.
class _PortraitButton extends StatelessWidget {
  const _PortraitButton({required this.character, required this.onEdit});

  final Character character;
  final void Function(CharacterEditCommand) onEdit;

  /// The game's own M portrait, so the picture sits at its native shape.
  ...
  Widget build(BuildContext context) {
    ...
    return Tooltip(
      message: 'Choose a different portrait',
      child: InkWell(
        onTap: () async {
          final chosen = await PortraitPicker.show(
            context,
            selected: character.portraitBaseName.isEmpty
                ? null
                : character.portraitBaseName,
          );
          if (chosen == null) return;
          onEdit(SetPortrait(creOffset: character.creOffset, baseName: chosen));
        },
        child: SizedBox(
          width: _width,
          height: _height,
          child: Stack(
            ...
            children: [
              ClipRRect(
                ...
                child: PortraitImage(
                  baseName: character.portraitBaseName,
                  iconSize: 32,
                ),
              ),
              // The affordance, because a picture is not obviously a button.
              Align(
      ...
      children: [
        _StatGroup(
          ...
          children: [
            ...
            if (reputation case final double party)
              _ReadOnlyStat(
...
class _Skills extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return _TabBody(
      children: [
        _ProficiencyGroup(character: character, sheet: sheet, onEdit: onEdit),
        _StatGroup(
...
/// an oversight.
class _ProficiencyGroup extends StatelessWidget {
