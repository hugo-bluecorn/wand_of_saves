  ...
  Widget build(BuildContext context) {
    ...
    return DecoratedBox(
      ...
      child: Padding(
        ...
        // ⚠️ **Both texts are `Flexible`, and that is not tidiness.** A tag is
        // a short word, so an unflexed `Row` looks safe — until one goes into a
        // 420-point side sheet beside a longer sibling and overflows by thirty
        // points. `mainAxisSize.min` keeps it hugging its content when there is
        // room; the flex only does anything when there is not.
        child: Row(
...
Tag? stateTagFor(SheetField field, {bool rulesBind = true}) {
  if (field.anomalous) {
